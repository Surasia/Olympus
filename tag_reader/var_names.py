...
from . tag_reader_utils import createDirAltNameID
...
def change_case(str):
    return reduce(lambda x, y: x + ('_' if y.isupper() and not x.isupper() else '') + y, str).lower()
...
def getStrInMmr3Hash(p_hash) -> str:
    ...
    if Mmr3Hash_str.keys().__contains__(p_hash):
        ...
        if not Mmr3Hash_str_iu.keys().__contains__(p_hash):
            ...
            path_to_hash = ROOT_DIR + '\\tag_reader\\hash\\in_use.txt'
...
def readTagNames():
    if TAG_NAMES == {}:
        path_to_hash = ROOT_DIR + '\\tag_reader\\tag_names\\tag_names_iu_generated.txt'
        with open(path_to_hash, 'rb') as f:
            hash_lines = f.readlines()
            for h_l in hash_lines:
                # print(h_l)
                #str_h_l = h_l.decode('ascii')
                str_h_l = h_l.decode('utf-8')
                temp_tuple = str_h_l.split(' : ')
                if len(temp_tuple) < 2:
                    continue
                key = temp_tuple[0]
                value = temp_tuple[1]
...
def loadInUseHashNamesFileJson():
    path_to_hash = ROOT_DIR + '\\tag_reader\\hash\\in_use.json'
    data = {}
    with open(path_to_hash, 'rb') as f:
        data = json.load(f)
...
def loadInUseHashNamesFileTxt():
    ...
    with open(path_to_hash, 'rb') as f:
        ...
        for h_l in hash_lines:
            ...
            value = temp_tuple[1]
            value = value.replace('\r\n', '')
            Mmr3Hash_str_iu[key] = value

    debug = True


def loadAlternativeHashNames():
    path_to_hash = ROOT_DIR + '\\tag_reader\\hash\\'
    for path in pathlib.Path(path_to_hash).rglob('*.txt'):
        if not (path.name.__contains__('web_generated')):
            continue
    ...
    for path in pathlib.Path(path_to_hash).rglob('*.txt'):
        ...
        with open(path, 'rb') as f:
            ...
            for h_l in hash_lines:
                ...
                if Mmr3Hash_alt.keys().__contains__(h_i):
                    if not Mmr3Hash_alt[h_i] == values[1]:
                        debug = Mmr3Hash_alt[h_i]
                        b = values[1]
                        if Mmr3Hash_alt[h_i].__contains__(f'-error-{h_i}'):
                            Mmr3Hash_alt[h_i] = values[1]
                        else:
