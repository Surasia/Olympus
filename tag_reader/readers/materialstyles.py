...
class MaterialStyles(BaseTemplate):
    ...
    def toJson(self, coatingname, root_folder):
        ...
        if style_select['name'].value == coatingname:
            ...
            self.json_base["name"] = style_select['palette'].ref_id_sub
            ...
            mwpl = style_select['palette'].ref_id_sub.lower()
            hex_bytes = bytes.fromhex(mwpl)
            reversed_bytes = hex_bytes[::-1]
            mwpl = reversed_bytes.hex()
            parse_mwpl = ReaderFactory.create_reader(self.root_folder+'mwpl/'+mwpl)
            parse_mwpl.load()
