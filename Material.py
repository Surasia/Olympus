...
class Material:
    ...
    #
    #
    def readMaterial(self,path,root,import_settings, add_material = False, material_name = None, material_prefab = None, use_modules = False):
        #with open(path,'rb') as fmat:
        fmat = ModulesManager.ModulesManagerContainer.manager.getFileHandle(path,use_modules)
        #print(path)
                ...
                for off in range(content_entry.data_reference.offset,content_entry.data_reference.offset + content_entry.data_reference.size,0x9c):
                    ...
                    if id == 0xe5562d34 and tag == b'mtib':
                        ...
                        if name in bpy.data.textures.keys() and import_settings.reuse_textures:
                            ...
                            tex = Texture()
                            if use_modules:
                                tex_path = "/__chore/pc__/" + self.string_table.strings[idx].replace("\\","/") + "{pc}.bitmap"
                            else:
                                tex_path = root + "/__chore/pc__/" + self.string_table.strings[idx].replace("\\","/") + "{pc}.bitmap"
                            tex.readTexture(tex_path, name,import_settings,use_modules)
