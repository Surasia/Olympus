...
class GrabStrings:
    def grabcoatingname(self, context):
        coatings = []
        try:
            with open(self.filepath, 'r') as coat:
                coatingdata = json.load(coat)
                for coating in coatingdata["coatings"]:
                    coatings.append((coating['name'], coating['name'], coating['name']))
                return coatings
        except:
            return [('', '', '')]
...
class ImportCoating(bpy.types.Operator, bpy_extras.io_utils.ImportHelper):
    ...
    coatingname: bpy.props.EnumProperty(items=GrabStrings.grabcoatingname, name = "Coating",  description = "Coating To Import",)
    ...
    def execute(self, context):
        ...
        # Addon Initialize
        parse_mwsy = open(self.filepath,'r')
        parse_mwsy = json.load(parse_mwsy)
        Nodes.betteruvscaling()
        Nodes.roughnessmath()
        Nodes.HIMS()
        
        # Style Parsing
        for coating in parse_mwsy["coatings"]:
            addon_prefs = context.preferences.addons[__package__].preferences
            self.root_folder = addon_prefs.root_folder.replace('\\', '/')
                        ...
                        if mat_slot.material.node_tree:
                            ...
                            for path in pathlib.Path(self.materialuserpath).rglob(materialname + '*_mat.json'): 
                                ...
                                if type(currentmaterial["style info"][0]) == type(None): # If material has no style, skip!
                                    ...
                                    materialstyleselection = self.filepath.split('\\')[-1].replace('_mat.json','').replace('.json','')
                                    normalmap = ""
                                    mask0 = 7443
                                    mask1 = 7443
                                    asgtexture = ""
