...
class GrabStrings:
    def grabcoatingnames(self, context):
        ...
        if type(parse_mwsy_name) == type(None):
            ...
            coatingnames = []
            if not type(parse_mwsy_name.tag_parse.rootTagInst) == type(None):
    ...
    def grabvisornames(self, context):
        ...
        parse_mwvs_name = ReaderFactory.create_reader(root_folder+'/mwvs/b4def045')
...
class ImportCoating(bpy.types.Operator, bpy_extras.io_utils.ImportHelper):
    ...
    materialuserpath: bpy.props.StringProperty(name = "Material Path", description = "Path To Use For .material",default = "M:\\mat \\")    
                        ...
                        for mat_slot in ob.material_slots: # get each material slot
                            if mat_slot.material.node_tree:
                                ...
                                if type(currentmaterial.get('StyleInfo')) == type(None): # If material has no style, skip!
                                    ...
                                    materialstyleselection = self.filepath.split('\\')[-1]
                                                        ...
                                                        if not swatchID == grimeSwatch:
                                                            ...
                                                            for visor in range(len(mappednames)):
                                                                ...
                                                                if colorID == self.visor_selection:
                                                                    parse_mvsw = ReaderFactory.create_reader(self.root_folder+'/mwvs/b4def045')
                                                                    parse_mvsw.load()
