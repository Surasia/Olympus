...
class renderModelImporter:
    def __init__(self):
        ...
        self.lod = 0
        self.scale_modifier = [1.0,1.0,1.0]
        self.root_folder = ""
        self.shader_file = ""
        self.shader_name = ""


    def processMesh(self, src_mesh, chunk_data, scale, scaleModifier, name, pos = (0.0,0.0,0.0), parent : bpy.types.Object = None):
    ...
    def openRenderModel(self,f, pos = (0.0,0.0,0.0), parent : bpy.types.Object = None):
        #header_length = 0x50
        #f.seek(0x18)
        ...
        for mesh_block in range(len(mesh_blocks)):
            ...
            self.processMesh(src_mesh,chunk_data, scale, scaleModifier,mesh_name, pos=pos,parent=parent)
...
class ImportRenderModel(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # set up the importer. This had to be moved outside the operator to be able to call it from different places too (BSP operator)
        rmi = renderModelImporter()
        rmi.filepath = self.filepath
        rmi.auto_import_dependencies = self.auto_import_dependencies
        rmi.import_uvs = self.import_uvs
        rmi.import_weights = self.import_weights
        rmi.import_normals = self.import_normals
        rmi.reuse_textures = self.reuse_textures
        rmi.add_materials = self.add_materials
        rmi.populate_shader = self.populate_shader
        rmi.import_model = self.import_model
        rmi.use_modules = self.use_modules
        rmi.mipmap = self.mipmap
        rmi.norm_signed = self.norm_signed
        rmi.lod = self.lod
        rmi.scale_modifier = self.scale_modifier
        rmi.root_folder = self.root_folder
        rmi.shader_file = self.shader_file
        rmi.shader_name = self.shader_name

        rmi.openRenderModel(f)
        f.close()
        return {'FINISHED'}
