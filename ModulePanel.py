...
class TREE_UL_List(bpy.types.UIList):
    ...
    def draw_item(self,context,layout,data,item,icon,active_data,active_propname,index):
        ...
        if item.type == "FILE":
            ...
            elif item.filename.split(".")[-1] == "material":
                #layout.label(text=item.filename,icon="FILE_IMAGE")
                op = layout.operator("infinite.infinitematerial",icon="MATERIAL",text=item.filename,emboss=False)
                op.filepath = item.path
                op.mipmap = importer_settings.mipmap
                op.use_modules = True
...
class ModulePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        MaterialSettings = self.layout.box()
        MaterialSettings.label(text="Material Settings")
        MaterialSettings.prop(bpy.context.scene.infinite_importer_settings,"mipmap")
